...
abstract class StageComponent{//the base class for all components that exsist inside a stage
  ...
  void draw(){};
  void draw3D(){};
  boolean colide(float x,float y,boolean c){return false;};//c= is colideing with click box
  boolean colide(float x,float y,float z){return false;};
  boolean colideDethPlane(float x,float Y){return false;};
  JSONObject save(boolean stage_3D){return null;};
